...
class ReportService {
  ...
  async generateExcel(userId) {
    ...
    // Alignment: Left for A, Right for B (Summary part)
    // Row 3 is 'THÔNG TIN CHUNG', Row 13 is 'Trạng thái'
    for (let i = 3; i <= 13; i++) {
      const row = sheet.getRow(i);
      row.getCell(1).alignment = { horizontal: 'left', vertical: 'middle' };
      row.getCell(2).alignment = { horizontal: 'right', vertical: 'middle' };
      row.height = 22; // Slightly taller rows for better look
    }
    ...
    // Sub-headers styling (Uniform for all sections)
    ...
    data.debts.forEach((d, idx) => {
      ...
      row.height = 25; // Increase height for data rows
      row.getCell(1).alignment = { horizontal: 'left', vertical: 'middle' };
  ...
  async generatePDF(userId, res) {
    ...
    // --- 2. REPORT TITLE & OWNER INFO (CENTERED) ---
    let currentY = 135;
    ...
    // Fixed Centering to avoid overlap
    ...
    doc.font('MainBold').text(nameStr, centerStartX + doc.widthOfString(labelStr), doc.y - 12); // Adjust Y for font baseline if needed
    ...
    // Domino Alert (Fixed Icon with safe character)
    if (data.summary.dominoAlerts.length > 0) {
      if (doc.y > 680) doc.addPage(); // Increased threshold from 600 to 680 to fit on Page 1
      const alertY = doc.y;
      ...
      doc.y = alertY + alertBoxHeight + 45; // Standardized gap
    } else {
      doc.y += 45; // Standardized gap
    }
    ...
    if (doc.y > 680) doc.addPage(); // Check BEFORE title to keep title with table
    ...
    // --- 6. KIẾN NGHỊ TỪ TRỢ LÝ FINSIGHT ---
