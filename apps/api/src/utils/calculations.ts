...
export function convertFlatToReducingAPR(principal: number, flatAPR: number, termMonths: number): number {
  ...
  while (err > tolerance && iterations < 100) {
...
/**
 * CALCULATION 1.6: Get effective APR (converts Flat → Reducing if needed)
 */
 ...
 * EAR = effectiveAPR + tổng phí thường niên quy đổi
 * Không dùng APY (lãi kép) vì FinSight áp dụng lãi đơn / reducing balance.
 */
