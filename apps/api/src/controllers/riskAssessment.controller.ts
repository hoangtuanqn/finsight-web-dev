...
export async function submitRiskAssessment(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // Use weight sent from frontend (per-question weight from the question pool).
    // Fall back to 1.0 if missing (backward compatibility).
    let weightedSum = 0;
    let totalWeight = 0;
    answers.forEach((a: any) => {
      const w = typeof a.weight === 'number' && a.weight > 0 ? a.weight : 1.0;
    ...
    const avgScore =
      totalWeight > 0
        ...
        : Math.round(answers.reduce((s: number, a: any) => s + (a.score || 0), 0) / answers.length);

    // Classify risk level using configured thresholds
    let riskLevel = 'LOW';
    ...
    if (avgScore >= RISK_CONFIG.thresholds.HIGH) {
      riskLevel = 'HIGH';
      riskDescription =
        'Bạn sẵn sàng chấp nhận rủi ro cao để tối đa hóa lợi nhuận. Phân bổ nhiều vào chứng khoán và crypto.';
    ...
    // Consistency check — warn user if answers are contradictory
