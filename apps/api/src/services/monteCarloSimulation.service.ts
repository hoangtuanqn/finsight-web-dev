...
export function boxMullerTransform(rng: () => number = Math.random): number {
  const u1 = Math.max(EPSILON, rng());
  const u2 = rng();
  return Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
}

export function choleskyDecomposition(matrix: any): number[][] | null {
...
export function generateProjectionTable(params: any) {
...
function publicMonteCarloPoint(result: any) {
  const point: any = { probLoss: result.probLoss };
...
export function buildBackwardCompatibleProjection(table: Record<string, any>) {
