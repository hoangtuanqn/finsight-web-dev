...
export const US_STOCK_UNIVERSE: UsStockMeta[] = [
  // Broad market ETF — phù hợp nhất với investor VN (passive, đa dạng hóa ngay)
  {
    ...
    sector: 'ETF Broad',
    tag: 'S&P 500 · 500 công ty lớn nhất Mỹ',
    isEtf: true,
  },
  {
    ticker: 'QQQ',
    ...
    sector: 'ETF Tech',
    tag: 'Nasdaq 100 · Tập trung Big Tech',
    isEtf: true,
  },
  {
    ticker: 'VTI',
    ...
    sector: 'ETF Broad',
    tag: 'Total Market · Phí thấp nhất',
    isEtf: true,
  },
  {
    ticker: 'IWM',
...
export function buildUsStockCard(quote: any, meta: UsStockMeta, rank: number): UsStockCard {
  ...
  if (rank === 0) {
    badge = 'Tốt nhất';
    badgeColor = 'amber';
  } else if (meta.sector === 'ETF Broad') {
    badge = 'Passive';
    badgeColor = 'blue';
  } else if (meta.sector === 'ETF Tech') {
    badge = 'Tech ETF';
    badgeColor = 'purple';
  } else if (change > 2) {
    badge = 'Đang tăng';
    badgeColor = 'emerald';
  } else if (posIn52w < 0.25) {
    badge = 'Vùng giá tốt';
    badgeColor = 'blue';
  } else if (posIn52w > 0.75) {
    badge = 'Uptrend';
    badgeColor = 'emerald';
  }
...
export async function getStocksUsPricesData(riskLevel: string): Promise<StocksUsServiceResult> {
  ...
  const scored = US_STOCK_UNIVERSE.filter((m) => quotes[m.ticker])
  ...
  const avgChange =
    quoteList.length > 0
      ? quoteList.reduce((sum, q) => sum + (q.prevClose > 0 ? ((q.price - q.prevClose) / q.prevClose) * 100 : 0), 0) /
        quoteList.length
      : 0;
  const marketMood =
    avgChange > 1 ? 'đang tăng 📈' : avgChange > 0 ? 'tích cực nhẹ' : avgChange > -1 ? 'đi ngang ➡️' : 'đang giảm 📉';
