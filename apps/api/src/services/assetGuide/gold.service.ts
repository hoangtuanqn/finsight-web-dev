...
const BRAND_CONFIG: Record<string, {
  ...
  sjc:          { label: 'SJC',               priority: 1, chartSlug: 'sjc',           barPattern: /1L.*10L|1 ch[ỉi].*2 ch[ỉi]/i,  ringPattern: /1 ch[ỉi].*2 ch[ỉi].*5 ch[ỉi]/i },
  ...
  btmc:         { label: 'Bảo Tín Minh Châu', priority: 4, chartSlug: 'baotinminhchau',barPattern: /SJC|VRTL/i,                     ringPattern: /Nhẫn.*999/i                    },
  phuquy:       { label: 'Phú Quý',           priority: 5, chartSlug: 'phuquy',        barPattern: /999\.9/i,                       ringPattern: /Nhẫn.*999/i                    },
  ngoctham:     { label: 'Ngọc Thẩm',         priority: 6, chartSlug: 'ngoctham',      barPattern: /999\.9|Ta.*999/i,               ringPattern: /Nhẫn.*999/i                    },
  kimnganphuc:  { label: 'Kim Ngân Phúc',     priority: 7, chartSlug: 'kimnganphuc',   barPattern: undefined,                       ringPattern: /Nhẫn.*9999|9999/i              },
};
...
function buildCostAnalysis(buyPrice: number, sellPrice: number, impliedPriceVnd: number): GoldCostAnalysis {
  ...
  const premiumPct        = impliedPriceVnd > 0 ? Math.round((sellPrice - impliedPriceVnd) / impliedPriceVnd * 10000) / 100 : 0;
  const totalEntryCostPct = Math.round((spreadPct + Math.max(premiumPct, 0)) * 100) / 100;
  const breakEvenPct      = totalEntryCostPct;
  const profitIfWorldUp3Pct  = impliedPriceVnd > 0
    ? Math.round(impliedPriceVnd * 1.03 - sellPrice - impliedPriceVnd * 1.03 * spreadPct / 100) : 0;
  const profitIfWorldUp10Pct = impliedPriceVnd > 0
    ? Math.round(impliedPriceVnd * 1.10 - sellPrice - impliedPriceVnd * 1.10 * spreadPct / 100) : 0;
  return { spreadPct, premiumPct, totalEntryCostPct, impliedPriceVnd, profitIfWorldUp3Pct, profitIfWorldUp10Pct, breakEvenPct };
}
...
function scoreGold(cost: GoldCostAnalysis, rsi14: number | null): number {
  ...
  return Math.round(costScore * 0.60 + rsiScore * 0.40);
}
...
async function fetchWorldGoldSpot(): Promise<{ price: number; change: number } | null> {
  try {
    const res = await fetch(
      ...
      { headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' }, signal: (AbortSignal as any).timeout(6000) }
    );
...
async function fetchGoldData() {
  ...
  // ── Tỷ giá ──
  const { rate: usdVnd, source: usdVndSource } = usdVndResult.status === 'fulfilled'
    ? usdVndResult.value : { rate: FALLBACK_USD_VND, source: 'fallback' };
  ...
  const impliedPerChi = worldPrice > 0 && usdVnd > 0
    ? Math.round(worldPrice * usdVnd / OZ_PER_CHI / 1000) * 1000 : 0;
...
export async function getGoldPricesData(): Promise<GoldServiceResult> {
  ...
  for (const [slug, cfg] of Object.entries(BRAND_CONFIG)) {
    ...
    for (const record of [barRecord, ringRecord].filter(Boolean)) {
      ...
      const posIn30dPct = (high30d > low30d && sellPerChi > 0)
        ? Math.round((sellPerChi - low30d) / (high30d - low30d) * 1000) / 10 : null;
      ...
      const tag  = isRing
        ? `Nhẫn tròn trơn · ${cfg.label}`
        : `Vàng miếng · ${cfg.label}`;
      ...
      brandItems.push({
        ...
        note: [
          ...
          cost.premiumPct > 0
            ? `Premium vs TG: +${cost.premiumPct}%${cost.premiumPct > 10 ? ' ⚠️' : ''}`
            : null,
  ...
  brandItems.sort((a, b) => b.score - a.score || (BRAND_CONFIG[a.id.split('_')[0]]?.priority ?? 99) - (BRAND_CONFIG[b.id.split('_')[0]]?.priority ?? 99));
  ...
  const rsiCtx     = bestRsi !== null
    ? (bestRsi <= 30 ? ' Đang ở vùng quá bán — cơ hội tích lũy.' : bestRsi >= 70 ? ' Đang ở vùng quá mua — cẩn thận vào mới.' : '') : '';
  const worldCtx   = worldPrice > 0
