...
const URGENCY: Record<string, any> = {
  danger: {
...
function getUrgency(daysUntil: number) {
  if (daysUntil <= 3)
    return {
      level: 'danger',
      label: daysUntil === 0 ? 'Hôm nay!' : `${daysUntil}n`,
    };
  if (daysUntil <= 7) return { level: 'warning', label: `${daysUntil} ngày` };
  return { level: 'normal', label: `${daysUntil} ngày` };
}

export default function DueDebts({ dueThisWeek = [] }: { dueThisWeek?: any[] }) {
    ...
    >
      ...
      <CardHeader
        ...
        action={
          ...
          >
            Xem tất cả {totalCount > 0 ? `(${totalCount})` : ''}{' '}
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[13px] font-bold text-[var(--color-text-primary)] truncate">{d.name}</p>
                        <p className="text-[10px] text-[var(--color-text-muted)]">{nextDueDateLabel(d.dueDay)}</p>
                      </div>
                    </div>
                    <p className="text-[13px] font-black shrink-0 ml-3" style={{ color: u.glow }}>
