  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-(--color-text-primary) tracking-tight">Kế hoạch Trả nợ Thông minh</h1>
          <p className="text-(--color-text-muted) mt-2 max-w-2xl font-medium">
            Tối ưu hóa dòng tiền doanh nghiệp bằng các chiến lược tài chính tiên tiến và theo dõi tuân thủ thực tế.
          </p>
        </div>

        <div className="flex bg-(--color-bg-secondary) p-1.5 rounded-2xl border border-(--color-border) shadow-inner">
          <button
                    ...
                    {STRATEGIES.map((s) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          >
                            <s.icon
                              className={`w-5 h-5 ${strategy === s.id ? s.color : 'text-(--color-text-muted)'}`}
                            />
                          </div>
            ...
            <AnimatePresence mode="wait">
              {!result ? (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  {/* Alerts & Optimal Suggestions Section */}
                  <div className="space-y-4">
                    ...
                    {result.optimalPoints && result.optimalPoints.length > 0 && (
                      <div className="p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 space-y-3">
                        <div className="flex items-center gap-2 text-blue-400">
                          <TrendingDown className="w-5 h-5" />
                          <h4 className="text-sm font-bold uppercase tracking-wider">Gợi ý tối ưu ngân sách</h4>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {result.optimalPoints.map((point, idx) => (
                            <button
                              key={idx}
                              onClick={() => setBudget(point.budget)}
                              className="p-3 rounded-xl bg-(--color-bg-secondary)/50 border border-blue-500/20 hover:border-blue-500/50 transition-all text-left group"
                            >
                              <p className="text-xs text-blue-400 font-bold">{formatCurrency(point.budget)}</p>
                              <p className="text-[10px] text-(--color-text-muted) mt-1 group-hover:text-(--color-text-primary)">
                                {point.impact}
                              </p>
                            </button>
                          ))}
