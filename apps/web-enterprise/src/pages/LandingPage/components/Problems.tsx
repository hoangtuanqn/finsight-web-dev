...
import { BrainCircuit, ChevronRight, CreditCard, Drama } from 'lucide-react';
import { fadeUp, GlowCard, GradientText, Section, StaggerGroup } from './Shared';
        ...
        {[
          ...
          <motion.div key={item.title} variants={fadeUp}>
            <GlowCard glowColor={item.glow} className="h-full group">
              <div
                className={`w-16 h-16 rounded-2xl ${item.iconColor} text-white flex items-center justify-center mb-8 shadow-2xl transition-transform group-hover:scale-110 group-hover:rotate-3 duration-500`}
              >
                <item.icon size={32} />
              </div>
