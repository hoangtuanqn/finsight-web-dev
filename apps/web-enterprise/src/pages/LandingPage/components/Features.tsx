...
import { ArrowUpRight, BarChart3, Bot, Check, Map } from 'lucide-react';
import { GlowCard, GradientText, Section, useIsMobile } from './Shared';
...
export default function Features() {
  ...
  const FeatureRow = ({
    ...
    <div
      className={`flex flex-col ${isReversed ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-12 lg:gap-20 mb-32 last:mb-0 relative z-10`}
    >
      <motion.div
      ...
      >
        <GlowCard glowColor={glowColor} className="!p-0 border-none bg-transparent">
          <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] p-8 lg:p-12 border border-white/60 dark:border-white/10 shadow-2xl relative overflow-hidden group">
            <div
              className={`absolute -top-24 -right-24 w-64 h-64 ${glowColor.replace('0.2', '0.1')} blur-[100px] rounded-full transition-transform duration-1000 group-hover:scale-150`}
            />
      ...
      >
        <div
          className={`w-16 h-16 rounded-2xl bg-white/50 dark:bg-white/5 backdrop-blur-md border border-white/60 dark:border-white/10 ${iconColor} flex items-center justify-center mb-8 shadow-2xl`}
        >
          <Icon size={32} />
        </div>
        <h3 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white mb-6 tracking-tighter leading-tight">
          {title}
        </h3>
        <p className="text-xl text-slate-600 dark:text-slate-400 mb-10 font-medium leading-relaxed">{desc}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {bullets.map((bullet, i) => (
            <div
              key={i}
              className="flex items-center gap-3 text-slate-700 dark:text-slate-300 font-bold text-sm bg-white/50 dark:bg-white/5 backdrop-blur-sm border border-white/60 dark:border-white/10 p-4 rounded-2xl hover:bg-white dark:hover:bg-white/10 transition-colors"
            >
    ...
    <Section id="features" className="py-32">
      <div className="text-center max-w-4xl mx-auto mb-32 relative">
        ...
        <h2 className="text-4xl md:text-7xl font-black text-slate-900 dark:text-white mb-6 relative z-10 tracking-tighter leading-tight">
          Giải pháp{' '}
          <GradientText from="from-blue-600" to="to-cyan-400">
            Tài chính
          </GradientText>
          <br /> công nghệ cao
        </h2>
      ...
      <FeatureRow
        ...
        bullets={['Nhận diện phí ẩn tự động', 'Mô phỏng chi phí lũy kế', 'Biểu đồ so sánh thời gian thực']}
      >
        <div className="space-y-8">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="font-black text-slate-900 dark:text-white tracking-wide uppercase text-xs">
                Risk Matrix Analyzer
              </div>
            </div>
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-white/5 px-3 py-1 rounded-full">
              Active Mode
            </div>
          </div>
          {[
            ...
            <div key={item.name} className="space-y-3 group">
              ...
              <div className="h-4 bg-slate-200/50 dark:bg-white/5 rounded-full overflow-hidden border border-white/20 dark:border-white/5 relative">
                <motion.div
                  ...
                  className={`h-full bg-gradient-to-r ${item.color} rounded-full shadow-[0_0_15px_rgba(59,130,246,0.3)]`}
                />
              </div>
            </div>
          ))}
      ...
      <FeatureRow
        ...
        bullets={['So sánh song song kịch bản', 'Tối ưu dòng tiền khả dụng', 'Lộ trình trực quan, dễ hiểu']}
      >
      ...
      <FeatureRow
        ...
        bullets={['Chỉ số Fear & Greed Real-time', 'Khuyến nghị rổ tài sản mượt', 'Cân bằng dòng tiền tự động']}
      >
        <div className="relative">
          <div className="flex justify-between items-end mb-10">
            <div>
              ...
              <div className="text-6xl font-black text-purple-600 tracking-tighter drop-shadow-[0_0_20px_rgba(167,139,250,0.4)]">
                FEAR
              </div>
            </div>
            <div className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter">
              42<span className="text-xl text-slate-400 font-bold ml-1">/100</span>
            </div>
          </div>
          ...
          <div className="p-6 rounded-[2rem] bg-purple-600/10 border border-purple-600/20 text-md font-bold text-purple-800 dark:text-purple-200 leading-relaxed shadow-lg">
            ...
            Thị trường đang trong giai đoạn sợ hãi. Đây là thời điểm tối ưu để gia tăng tỷ trọng tài sản an toàn (Vàng,
            Tiền gửi) và DCA kỷ luật vào các mã blue-chip.
          </div>
        </div>
      </FeatureRow>
    </Section>
