...
import { BarChart3, ChevronRight, FileEdit, Globe, Rocket, ShieldCheck, Zap } from 'lucide-react';
import { AnimatedCounter, GlowCard, GradientText, Section, StaggerGroup, fadeUp } from './Shared';
            ...
            {[
              {
                ...
                sub: 'Credit Cards, E-wallets, Loans',
              },
              {
                icon: ShieldCheck,
                value: 100,
                ...
                sub: 'Non-custodial information storage',
              },
              ...
              <motion.div key={i} variants={fadeUp} className="group flex flex-col items-center">
                ...
                <div className="text-xl font-black text-slate-800 dark:text-slate-200 mb-2 uppercase tracking-widest">
                  {item.label}
                </div>
          ...
          {[
            {
              ...
              desc: 'Đồng bộ hóa dữ liệu tài chính qua giao diện trực quan. Nhập thông tin các khoản nợ với vài thao tác cơ bản.',
            ...
            },
            {
              ...
              icon: BarChart3,
              title: 'Core Analysis',
              desc: 'Engine của FinSight bóc tách cấu trúc nợ, chi phí ẩn và trả về kịch bản tối ưu hóa dòng tiền.',
            ...
            },
            {
              step: '03',
              icon: Rocket,
              title: 'Growth Staking',
              desc: 'Thực thi lộ trình thoát nợ và dịch chuyển dòng tiền sang tích lũy tài sản theo hướng dẫn từ AI.',
            ...
            <motion.div key={item.step} variants={fadeUp} className="relative z-10">
              <GlowCard glowColor={item.glow} className="h-full text-center group">
                ...
                <div
                  className={`relative w-24 h-24 mx-auto rounded-[2rem] bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 ${item.color} flex items-center justify-center mb-10 shadow-2xl transition-transform group-hover:rotate-12 group-hover:scale-110 duration-500`}
                >
                  <div
                    className={`absolute inset-0 ${item.glow} blur-xl rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity`}
                  />
                  <item.icon size={44} className="relative z-10" />
                </div>

                <h4 className="text-2xl font-black text-slate-900 dark:text-white mb-6 tracking-tight uppercase">
                  {item.title}
                </h4>
