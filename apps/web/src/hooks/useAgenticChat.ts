...
import { streamChat, getSessions, getSessionMessages, deleteSession } from '../api/agentic';
...
export function useAgenticChat() {
  ...
  const sendMessage = useCallback(async (text: string, ocrText: string | null = null, overrideDisplay: string | null = null) => {
    if (!text.trim() || isStreaming) return;

    const displayContent = overrideDisplay || text;
    const userMsg: Message = { id: `user-${Date.now()}`, role: 'user', content: displayContent };
