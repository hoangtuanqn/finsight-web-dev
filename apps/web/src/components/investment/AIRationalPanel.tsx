...
import { ASSET_LABELS, ASSET_ICONS } from "./InvestmentConstants";
import { explainAsset } from "./InvestmentUtils";
...
export default function AIRationalPanel({
  ...
  const riskLabel =
    ...
    "-";
  const horizonLabel =
    { SHORT: "Ngắn hạn", MEDIUM: "Trung hạn", LONG: "Dài hạn" }[
      profile?.horizon
    ] || "-";
  const goalLabel =
    {
      STABILITY: "Ổn định",
      SPECULATION: "Tăng trưởng",
      GROWTH: "Tăng trưởng",
      INCOME: "Thu nhập",
    }[profile?.goal] || "-";
    ...
    <div className="bg-slate-900/60 backdrop-blur-xl border border-white/5 p-5 rounded-3xl shadow-[0_8px_32px_-8px_rgba(0,0,0,0.5)] relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative z-10">
        {sorted.map(([asset, pct]) => {
          ...
          const breakdown = portfolioBreakdown.find(
            (p) => p.asset === ASSET_LABELS[asset],
          );

          return (
            <div
              key={asset}
              className={`border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 rounded-xl overflow-hidden ${isOpen ? "ring-1 ring-blue-500/30" : ""}`}
            >
              <button
...
function Badge({ label, value, color }) {
  ...
  return (
    <div
      className={`px-3 py-1.5 border rounded-full flex items-center gap-2 ${colors[color] || colors.blue}`}
    >
