...
import { Sparkles, BarChart2, Clock, AlertTriangle, ChevronDown, X, Loader2 } from 'lucide-react';
        ...
        {active && suggestion && (
          ...
          >
            <div className="mb-8 p-5 rounded-2xl bg-white/[0.02] border border-white/5 relative overflow-hidden">
              <div className="absolute left-0 top-0 bottom-0 w-1.5 rounded-l-2xl" style={{ background: suggestion.color, boxShadow: `0 0 15px ${suggestion.color}` }} />
                  ...
                  <div className="space-y-4">
                    ...
                    {best && (
                      <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/5">
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black bg-amber-500/20 border border-amber-500/30 text-amber-400 shrink-0">1</div>
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-white">{best.name}</p>
                          <p className="text-[11px] text-slate-400 mt-0.5">
                            Spread <span className="text-slate-200 font-semibold">{best.costAnalysis?.spreadPct}%</span>
                            {' · '}Chi phí vào lệnh <span className={`font-semibold ${best.costAnalysis?.totalEntryCostPct > 10 ? 'text-amber-400' : 'text-slate-200'}`}>{best.costAnalysis?.totalEntryCostPct}%</span>
                            {best.costAnalysis?.premiumPct > 0 && <> · Premium vs TG <span className={best.costAnalysis.premiumPct > 10 ? 'text-amber-400 font-semibold' : 'text-slate-200 font-semibold'}>+{best.costAnalysis.premiumPct}%</span></>}
                          </p>
                        </div>
                        <span className="ml-auto text-sm font-bold shrink-0" style={{ color: suggestion.color }}>{best.priceLabel}</span>
                      </div>
                    )}
                    ...
                    {(() => {
                      ...
                      const lowestSpread = withCost.length ? Math.min(...withCost.map((it: any) => it.costAnalysis.spreadPct)) : null;
                      const lowestPremium = withCost.length ? Math.min(...withCost.map((it: any) => it.costAnalysis.premiumPct)) : null;
                      const lowestCost = withCost.length ? Math.min(...withCost.map((it: any) => it.costAnalysis.totalEntryCostPct)) : null;
                      const chips = [
                        m.impliedPerChi > 0      && { label: 'Giá TG quy đổi',    value: `${m.impliedPerChi.toLocaleString('vi-VN')}đ/chỉ`,                                   color: 'text-slate-200' },
                        activeData.worldPrice > 0 && { label: 'Vàng thế giới',     value: `$${activeData.worldPrice.toLocaleString('en-US', { maximumFractionDigits: 0 })}/oz`, color: 'text-slate-200' },
                        m.usdVnd > 0             && { label: 'Tỷ giá USD/VND',     value: m.usdVnd.toLocaleString('vi-VN'),                                                     color: 'text-slate-200' },
                        lowestSpread !== null     && { label: 'Spread thấp nhất',   value: `${lowestSpread}%`,                                                                   color: 'text-emerald-400' },
                        lowestPremium !== null    && { label: 'Premium thấp nhất',  value: `+${lowestPremium}%`,                                                                 color: lowestPremium > 10 ? 'text-amber-400' : 'text-emerald-400' },
                        lowestCost !== null       && { label: 'Chi phí thấp nhất',  value: `${lowestCost}%`,                                                                     color: lowestCost > 10 ? 'text-amber-400' : 'text-emerald-400' },
                        false, // removed: Thương hiệu theo dõi
                      ].filter(Boolean) as { label: string; value: string; color: string }[];
                      return (
                        <div className="flex flex-wrap gap-2">
                          {chips.map(({ label, value, color }) => (
                            <div key={label} className="flex flex-col px-3 py-2 rounded-xl bg-white/[0.03] border border-white/5">
                    ...
                    <div className="rounded-xl border border-white/5 bg-white/[0.02] overflow-hidden">
                      <div className="px-4 py-2.5 border-b border-white/5 flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Cách FinSight xếp hạng vàng</span>
                      </div>
                      <div className="px-4 py-3 flex gap-6">
                        <div className="flex items-start gap-2 flex-1">
                          <span className="shrink-0 text-[10px] font-black px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 border border-amber-500/20 mt-0.5">60%</span>
                          <div>
                            <p className="text-[11px] font-bold text-slate-200">Chi phí vào lệnh</p>
                            <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">Spread + Premium so với giá thế giới. Thấp = dễ hoà vốn, xếp hạng cao hơn.</p>
                          </div>
                        </div>
                        <div className="w-px bg-white/5 shrink-0" />
                        <div className="flex items-start gap-2 flex-1">
                          <span className="shrink-0 text-[10px] font-black px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/20 mt-0.5">40%</span>
                          <div>
                            <p className="text-[11px] font-bold text-slate-200">RSI 14 ngày</p>
                            <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">RSI ≤30 = vùng đáy, cơ hội tích lũy. RSI ≥70 = quá nóng, cẩn thận vào mới.</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        >
          ...
          <div className="px-5 pb-4 grid gap-1.5" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
            {[
              ...
              { label: 'Bán ra',         value: item.price     ? formatVND(item.price)     : null, valueColor: '', valueStyle: { color } },
              { label: 'Spread',         value: item.costAnalysis?.spreadPct        != null ? `${item.costAnalysis.spreadPct}%`        : null, valueColor: 'text-white' },
              { label: 'Chi phí lệnh',   value: item.costAnalysis?.totalEntryCostPct != null ? `${item.costAnalysis.totalEntryCostPct}%` : null, valueColor: item.costAnalysis?.totalEntryCostPct > 10 ? 'text-amber-400' : 'text-white' },
              { label: 'Premium TG',     value: item.costAnalysis?.premiumPct > 0   ? `+${item.costAnalysis.premiumPct}%`              : null, valueColor: item.costAnalysis?.premiumPct > 10 ? 'text-amber-400' : 'text-slate-300' },
            ].map(({ label, value, valueColor, valueStyle }) => value && (
              <div key={label} className="flex flex-col items-center text-center px-1.5 py-2 rounded-xl bg-white/[0.03] border border-white/5">
                <span className="text-[9px] font-semibold text-slate-500 mb-1 leading-tight">{label}</span>
                <span className={`text-[11px] font-bold leading-tight ${valueColor}`} style={valueStyle}>{value}</span>
              </div>
            ))}
          </div>
...
function AssetCard({ item, color, type, rank, onOpenModal }) {
  ...
  return (
    <motion.div
      ...
      onClick={() => { if (canShowHistory) onOpenModal(item); }}
      onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && canShowHistory) { e.preventDefault(); onOpenModal(item); } }}
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            <span className={`shrink-0 w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black border ${
              rank === 1 ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' :
              rank === 2 ? 'bg-slate-400/10 border-slate-400/20 text-slate-300' :
              rank === 3 ? 'bg-orange-700/20 border-orange-700/30 text-orange-500' :
      ...
      <div className="flex flex-wrap gap-1.5 relative z-10">
        ...
        {cost && item.price != null && (
          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-white/[0.04] border border-white/5" style={{ color }}>
