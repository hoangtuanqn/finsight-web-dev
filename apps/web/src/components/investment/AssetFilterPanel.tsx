...
export default function AssetFilterPanel({
  ...
  const selectedCount = ASSET_CONFIGS.length - excludedAssets.length - 1; // -1 for savings
  ...
  return (
    <div
      className={[
        ...
        locked ? 'border-white/5 bg-slate-900/30' : 'border-white/8 bg-slate-900/50 backdrop-blur-sm',
      ].join(' ')}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5">
          <div
            className={['w-1.5 h-1.5 rounded-full', locked ? 'bg-slate-500' : 'bg-emerald-400 animate-pulse'].join(' ')}
          />
          ...
          return (
            <motion.button
              ...
              className={[
                ...
                selected ? `${asset.bgColor} ${asset.borderColor}` : 'bg-slate-800/60 border-white/5 opacity-35',
                locked
                  ? 'cursor-default'
                  : isAlwaysLocked
                    ? 'cursor-default'
            ...
            >
              ...
              {!locked && isAlwaysLocked && <Lock size={10} className="absolute top-1.5 right-1.5 text-slate-500" />}
