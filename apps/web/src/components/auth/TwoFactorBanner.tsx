...
import { ArrowRight, CheckCircle2, Monitor, ShieldAlert, ShieldCheck, Sparkles, X } from 'lucide-react';
      ...
      >
        {state === 'NONE' ? (
          ...
          <div className="relative group overflow-hidden rounded-[2rem] p-[1px] bg-gradient-to-r from-rose-500/30 via-purple-500/30 to-indigo-500/30 shadow-lg shadow-rose-500/5">
            <div className="relative bg-slate-900/90 backdrop-blur-2xl rounded-[1.95rem] px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 font-medium">
                    Bật 2FA ngay để bảo vệ tài sản của bạn trước các rủi ro bảo mật.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Link
                  to="/profile"
                  className="px-5 py-2.5 rounded-xl bg-white text-slate-950 text-[10px] font-black uppercase tracking-widest hover:bg-rose-50 transition-all flex items-center gap-2 group"
                >
                  Thiết lập ngay <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </Link>
                <button
                  onClick={handleDismiss}
                  className="p-2.5 rounded-xl text-slate-500 hover:text-white transition-all"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex flex-col lg:flex-row items-center gap-6 relative z-10">
                ...
                <div className="flex-1 text-center lg:text-left">
                  ...
                  <p className="text-slate-400 text-[11px] font-medium leading-relaxed max-w-lg">
                    FinSight nhận diện đây là trình duyệt an toàn. Bạn có muốn bỏ qua 2FA tại đây không?
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    onClick={handleDismiss}
                    className="px-5 py-3 rounded-xl bg-white/5 border border-white/10 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-white transition-all"
                  >
                    Lúc khác
                  </button>
                </div>
          ...
          <div className="inline-flex items-center gap-3 px-4 py-2 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 backdrop-blur-md">
            ...
            <button onClick={handleDismiss} className="ml-2 p-1 text-slate-500 hover:text-white transition-all">
              <X size={12} />
            </button>
          </div>
        )}
