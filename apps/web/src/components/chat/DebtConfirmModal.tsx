...
export default function DebtConfirmModal({ data, onConfirm, onDismiss }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      balance: debtInfo.balance
        ? String(debtInfo.balance)
        : amt
          ? String(amt)
          : "",
      apr:
        debtInfo.apr !== undefined && debtInfo.apr !== null
          ? String(debtInfo.apr)
          : "",
      termMonths: term ? String(term) : "",
      minPayment: debtInfo.minPayment
        ? String(debtInfo.minPayment)
        : amt && term
          ? String(Math.round(amt / term))
          : "",
      startDate: debtInfo.startDate || today,
      dueDay: debtInfo.dueDay ? String(debtInfo.dueDay) : "15",
      feeProcessing: debtInfo.feeProcessing
        ? String(debtInfo.feeProcessing)
        : "0",
      feeInsurance: debtInfo.feeInsurance ? String(debtInfo.feeInsurance) : "0",
      feeManagement: debtInfo.feeManagement
        ? String(debtInfo.feeManagement)
        : "0",
  ...
  const validate = () => {
    ...
    if (!+form.originalAmount || +form.originalAmount <= 0)
      return "Số tiền gốc phải lớn hơn 0.";
    if (!+form.balance || +form.balance <= 0)
      return "Dư nợ hiện tại phải lớn hơn 0.";
    if (+form.balance > +form.originalAmount)
      return "Dư nợ không được lớn hơn số tiền gốc.";
    ...
    if (!+form.termMonths || +form.termMonths <= 0)
      return "Kỳ hạn phải lớn hơn 0.";
    if (!+form.minPayment || +form.minPayment <= 0)
      return "Số tiền trả tối thiểu/tháng phải lớn hơn 0.";
    if (+form.minPayment > +form.balance)
      return "Số tiền trả tối thiểu không được lớn hơn dư nợ.";
    if (!form.startDate) return "Vui lòng chọn ngày vay.";
    if (new Date(form.startDate) > new Date())
      return "Ngày vay không được ở tương lai.";
    if (!+form.dueDay || +form.dueDay < 1 || +form.dueDay > 31)
      return "Ngày đáo hạn phải từ 1 đến 31.";
    return null;
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.error || "Lỗi khi lưu khoản nợ. Vui lòng thử lại.";
      setError(msg);
    } finally {
      setLoading(false);
    }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <span
                    className={
                      form.rateType === "FLAT" ? "text-amber-400 font-bold" : ""
                    }
                  >
                    Lãi suất phẳng
                  </span>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateField("feeProcessing", e.target.value)
                              }
                              min="0"
                              max="100"
                              step="0.1"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateField("feeInsurance", e.target.value)
                              }
                              min="0"
                              max="100"
                              step="0.1"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateField("feeManagement", e.target.value)
                              }
                              min="0"
                              max="100"
                              step="0.1"
