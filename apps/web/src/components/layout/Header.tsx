...
function IconBtn({ onClick, title, badge, children, id }: { onClick: () => void, title?: string, badge?: number, children: React.ReactNode, id?: string }) {
  return (
    <button
      id={id}
      onClick={onClick}
...
export default function Header({ sidebarWidth = 260, isCollapsed, setIsCollapsed, isMobile }: HeaderProps) {
  ...
  useEffect(() => {
    const fetch = async () => { try { setNotifs((await (userAPI as any).getNotifications()).data.data.notifications || []); } catch (_) {} };
    fetch();
  ...
  const markAll = async () => { try { await (userAPI as any).markAllRead(); setNotifs(p => p.map(n => ({ ...n, isRead: true }))); } catch (_) {} };
  const markOne = async (id: string) => { try { await (userAPI as any).markRead(id); setNotifs(p => p.map(n => n.id === id ? { ...n, isRead: true } : n)); } catch (_) {} };
