...
export default function RiskAssessmentPage() {
  ...
  const handleAnswer = async (option: any) => {
    const newAnswers = [
      ...answers,
      { questionIndex: current, score: option.score },
    ];
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h2 className="text-xl font-black text-[var(--color-text-primary)] mb-1">
            Kết quả đánh giá
          </h2>
          <p className="text-sm text-[var(--color-text-muted)] mb-6">
            Dựa trên {QUESTIONS.length} câu trả lời của bạn
          </p>
          ...
          >
            <p
              className={`text-4xl font-black bg-gradient-to-r ${meta.gradient} bg-clip-text text-transparent mb-2`}
            >
              {meta.label}
            </p>
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="max-w-2xl mx-auto py-4 pb-8"
    >
      {/* Header */}
      <div className="mb-6">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <h2 className="text-[16px] font-black text-[var(--color-text-primary)] mb-6 leading-relaxed">
            {q.question}
          </h2>
