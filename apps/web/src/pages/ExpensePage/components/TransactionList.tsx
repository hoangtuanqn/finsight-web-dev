      ...
      <div className="p-16 text-center space-y-4 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-3xl shadow-sm">
        ...
        <div>
          ...
          <p className="text-[13px] text-[var(--color-text-muted)] mt-1">
            Ghi chép chi tiêu để quản lý tài chính tốt hơn.
          </p>
        </div>
      </div>
    );
          ...
          <div className="divide-y divide-[var(--color-border)]">
            {group.items.map((item: any) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[11px] font-bold text-[var(--color-text-muted)]">{item.category.name}</span>
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-[15px] font-black ${item.type === 'INCOME' ? 'text-emerald-500' : 'text-red-500'}`}
                  >
                    {item.type === 'INCOME' ? '+' : '-'}
                  ...
                  <ChevronRight
                    size={16}
                    className="text-[var(--color-text-muted)] opacity-0 group-hover:opacity-100 transition-opacity -mr-2"
                  />
                </div>
              </button>
            ))}
