...
export function WalletForm({ wallet, onClose }: WalletFormProps) {
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (type !== 'BANK') {
      ...
      setForm((f) => ({ ...f, type, bankName: '', bankAccountNumber: '', sepayToken: '' }));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
      ...
      bankName: isBank ? form.bankName || null : null,
      bankAccountNumber: isBank ? form.bankAccountNumber || null : null,
      sepayToken: isBank && syncEnabled ? form.sepayToken || null : null,
    };
    wallet
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
      ...
      >
        ...
        />
        <div
          className="absolute -top-20 left-1/2 -translate-x-1/2 w-60 h-32 rounded-full blur-3xl opacity-20 pointer-events-none"
        ...
        <div className="shrink-0 px-6 pt-6 pb-5 border-b border-[var(--color-border)]">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              <div
                className="w-9 h-9 rounded-2xl flex items-center justify-center text-lg shadow-inner"
                style={{ background: `${form.color}20`, border: `1px solid ${form.color}30` }}
              >
                {form.icon}
              </div>
              <div>
                <h2 className="text-[15px] font-black text-[var(--color-text-primary)]">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-xl text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-secondary)] transition-all"
            >
              <X size={16} />
            </button>
          </div>
          ...
          >
            <div
              ...
              style={{ background: `linear-gradient(90deg,transparent,${form.color}60,transparent)` }}
            />
            <div className="flex items-center justify-between">
              <div>
                <p
                  className="text-[11px] font-black uppercase tracking-widest mb-1"
                  style={{ color: `${form.color}99` }}
                >
                  Xem trước
                </p>
                <p className="text-[15px] font-black text-[var(--color-text-primary)]">
                  {form.name || 'Tên ví của bạn'}
                </p>
                {isBank && selectedBank && (
                  <p className="text-[11px] text-[var(--color-text-muted)] mt-0.5">{selectedBank.name}</p>
                )}
              </div>
              <div className="text-right">
                <p className="text-[18px] font-black" style={{ color: form.color }}>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {WALLET_ICONS.map((icon) => (
                  <button
                    ...
                    style={
                      form.icon === icon ? { background: `${form.color}20`, border: `2px solid ${form.color}60` } : {}
                    }
                  >
                    {icon}
                  </button>
          ...
          <div>
            ...
            <input
              ...
              placeholder="VD: TPBank, Bóp tiền mặt..."
              required
          ...
          <AnimatePresence>
            {isBank && (
              ...
              >
                ...
                <motion.div
                  animate={{
                    borderColor: syncEnabled ? 'rgba(59,130,246,0.35)' : 'var(--color-border)',
                    background: syncEnabled ? 'rgba(59,130,246,0.06)' : 'var(--color-bg-secondary)',
                  }}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ background: syncEnabled ? 'rgba(59,130,246,0.15)' : 'var(--color-bg-primary)' }}
                      >
                    ...
                    {syncEnabled && (
                      ...
                      >
                          ...
                          <div className="relative">
                            ...
                            >
                              ...
                              <ChevronDown
                                ...
                                style={{ transform: showBankDropdown ? 'rotate(180deg)' : '' }}
                              />
                            </button>
                            <AnimatePresence>
                              {showBankDropdown && (
                                  ...
                                  <div className="overflow-y-auto" style={{ maxHeight: 200 }}>
                                    {filteredBanks.map((bank) => (
                                      <button
                                        ...
                                        type="button"
                                        onClick={() => {
                                          setForm((f) => ({ ...f, bankName: bank.code }));
                                      ...
                                      >
                                        ...
                                        {form.bankName === bank.code && (
                                          <Check size={13} className="text-blue-500 shrink-0 ml-2" />
                                        )}
                                      </button>
                                    ))}
                                    {filteredBanks.length === 0 && (
                                      <p className="text-center text-[12px] text-[var(--color-text-muted)] py-6">
                        ...
                        <div>
                          ...
                          <input
                            value={form.sepayToken}
                            onChange={(e) => set('sepayToken', e.target.value)}
                            type="password"
                            autoComplete="off"
                        ...
                        >
                          ...
                          >
                            ...
                            <ChevronDown
                              ...
                              style={{ transform: showGuide ? 'rotate(180deg)' : '' }}
                            />
                          </button>
                          <AnimatePresence>
                            {showGuide && (
                                ...
                                <div className="px-3 pb-3 space-y-1.5">
                                  {[
                                    ...
                                    <>
                                      ...
                                      và liên kết tài khoản ngân hàng.
                                    </>,
                                    <>
                                      Sao chép token và dán vào ô trên. Hệ thống đồng bộ mỗi{' '}
                                      <span className="font-bold text-blue-400">10 giây</span>.
                                    </>,
                                  ].map((step, i) => (
                                    <div key={i} className="flex items-start gap-2">
                                      <span className="w-4 h-4 rounded-full bg-blue-500/20 text-blue-400 text-[9px] font-black flex items-center justify-center shrink-0 mt-0.5">
                                        {i + 1}
                                      </span>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: `0 8px 24px ${form.color}35`,
            }}
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              {isPending ? (
                <>
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Đang
                  lưu...
                </>
              ) : wallet ? (
                '✓ Cập nhật ví'
