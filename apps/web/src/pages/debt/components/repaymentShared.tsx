  ...
  return (
    <div className="rounded-2xl border border-red-400/25 bg-red-500/8 p-4 text-[13px] text-red-100/90 leading-relaxed">
      <div className="flex items-start gap-2.5">
        ...
        <p>
          Khoản nợ <span className="font-black text-red-100">{breach.name}</span> bị quá hạn vào tháng thứ{' '}
          {breach.month} khi trả nợ theo phương pháp {method}. Còn {formatVND(breach.remainingBalance)} sau kỳ hạn{' '}
...
export const STRATEGY_CONTENT = {
  AVALANCHE: {
    ...
    worstFor: 'Người cần động lực liên tục hoặc dễ bỏ cuộc khi chưa thấy kết quả sớm.',
  },
  SNOWBALL: {
    name: 'Snowball',
    ...
    bestFor: 'Người cần động lực thường xuyên hoặc đang có nhiều khoản nợ nhỏ cần dọn sạch.',
    worstFor: 'Người có khoản nợ lớn với lãi suất rất cao, vì Avalanche có thể tiết kiệm hơn đáng kể.',
  },
};

export type StrategyType = keyof typeof STRATEGY_CONTENT;

export function StrategyModal({ type, onClose }: { type: StrategyType; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`absolute top-0 left-8 right-8 h-px bg-gradient-to-r from-transparent ${tone.glow} to-transparent`}
        />
            ...
            <ol className="space-y-2">
              {content.howItWorks.map((step, i) => (
                <li key={step} className="flex items-start gap-2.5">
                  <span
                    className={`shrink-0 w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center mt-0.5 ${tone.number}`}
                  >
                    {i + 1}
                  </span>
      ...
      <ul className="space-y-1.5">
        {items.map((item) => (
          <li key={item} className="flex items-start gap-1.5">
            ...
            <span className="text-[11px] text-[var(--color-text-secondary)] leading-relaxed">{item}</span>
          </li>
        ))}
      </ul>
    </div>
...
function StrategyFit({
  ...
  return (
    <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-white/4 border border-white/6">
      {icon}
      <div>
        <span className={`text-[10px] font-black uppercase tracking-wide ${strategyTone[tone].title}`}>{label} </span>
        <span className="text-[12px] text-[var(--color-text-secondary)]">{text}</span>
      </div>
    </div>
  );
}
...
export function MethodPlanCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-5 right-5 h-px bg-gradient-to-r from-transparent ${tone.glow} to-transparent`}
      />
      <div className="flex items-center gap-2.5 mb-2">
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${tone.iconBox}`}>{tone.icon}</div>
        <h3 className={`font-black text-[16px] ${tone.text}`}>{tone.name}</h3>
        <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${tone.iconBox}`}>{tone.badge}</span>
        {onInfo && (
          <button
            type="button"
            onClick={onInfo}
      ...
      <p className="text-[12px] text-[var(--color-text-muted)] mb-5">{tone.subtitle}</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div>
              ...
              <p className="text-2xl font-black text-[var(--color-text-primary)]">
                {simulation.months} <span className="text-sm text-[var(--color-text-muted)] font-medium">tháng</span>
              </p>
            </div>
            <div>
              <p className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wider font-bold mb-1">
              ...
              <p className="text-xl font-black text-red-400">{formatVND(simulation.totalInterest)}</p>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {orderedDebts.map((debt, index) => (
                  <div key={debt.id} className="flex items-center gap-2.5">
                    <div
                      className={`shrink-0 w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${index === 0 ? tone.iconBox : 'bg-white/5 text-[var(--color-text-muted)]'}`}
                    >
                      {index + 1}
                    </div>
                    <Link
