...
function StrategyModal({ type, onClose }: { type: keyof typeof STRATEGY_CONTENT, onClose: () => void }) {
...
export default function RepaymentPlanPage() {
  ...
  const [modal, setModal] = useState<keyof typeof STRATEGY_CONTENT | null>(null);
  ...
  const { data: planData, isLoading: planLoading } = useRepaymentPlan(debouncedBudget) as { data: any, isLoading: boolean };
  const { data: debtsData, isLoading: debtsLoading } = useDebts() as { data: any, isLoading: boolean };
  const { data: goalData, isLoading: goalLoading } = useDebtGoal() as { data: any, isLoading: boolean };
