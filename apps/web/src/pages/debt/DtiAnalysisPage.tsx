...
import { AlertOctagon, AlertTriangle, CheckCircle, Info, Lightbulb, TrendingUp } from 'lucide-react';
...
export default function DtiAnalysisPage() {
  const { data, isLoading: loading } = useDtiAnalysis() as {
    data: any;
    isLoading: boolean;
  };
  ...
  const zone = ZONE_CONFIG[summary.zone as keyof typeof ZONE_CONFIG] || ZONE_CONFIG.SAFE;
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            gradient: summary.remainingCashflow >= 0 ? 'from-emerald-500 to-teal-400' : 'from-red-500 to-rose-400',
          ...
          >
            ...
            />
            <p className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest mb-3">
              {item.label}
            </p>
            <p className={`text-2xl font-black bg-gradient-to-r ${item.gradient} bg-clip-text text-transparent mb-1`}>
              {item.value}
            </p>
      ...
      >
        <div
          className="absolute top-0 left-5 right-5 h-px"
          style={{
            background: `linear-gradient(90deg,transparent,${zone.color}50,transparent)`,
          }}
        />
      ...
      {breakdown.length > 0 && (
        ...
        >
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <Link
                      to={`/debts/${debt.id}`}
                      className="text-[13px] font-bold text-[var(--color-text-primary)] hover:text-blue-400 transition-colors cursor-pointer"
                    >
                      {debt.name}
                    </Link>
                    <p className="text-[11px] text-[var(--color-text-muted)]">
                      {debt.platform} • {formatVND(debt.minPayment)}/tháng
                    </p>
                  </div>
                  <span className="text-[13px] font-black" style={{ color: zone.color }}>
                ...
                <div className="h-1.5 rounded-full bg-[var(--color-bg-secondary)] overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.min(100, (debt.dtiContribution / Math.max(summary.dtiRatio, 1)) * 100)}%`,
                    }}
          ...
          <div className="mt-5 pt-4 border-t border-[var(--color-border)] flex justify-between text-[13px]">
            ...
            <span className="font-black" style={{ color: zone.color }}>
        ...
        >
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-emerald-500/6 border border-emerald-500/15">
              <div className="w-6 h-6 rounded-lg bg-emerald-500/20 flex items-center justify-center text-[11px] font-black text-emerald-400 shrink-0">
                ①
              </div>
              <div>
                <p className="text-[13px] text-[var(--color-text-primary)]">
                  Tăng thu nhập lên{' '}
                  <span className="font-black text-emerald-400">{formatVND(whatIf.incomeNeededForSafe)}/tháng</span>
                </p>
                <p className="text-[11px] text-[var(--color-text-muted)] mt-0.5">
                  Tăng thêm {formatVND(Math.max(0, whatIf.incomeNeededForSafe - summary.monthlyIncome))}
                  /tháng
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-blue-500/6 border border-blue-500/15">
              <div className="w-6 h-6 rounded-lg bg-blue-500/20 flex items-center justify-center text-[11px] font-black text-blue-400 shrink-0">
                ②
              </div>
              <div>
                <p className="text-[13px] text-[var(--color-text-primary)]">
                  Giảm số tiền trả nợ xuống{' '}
                  <span className="font-black text-blue-400">{formatVND(summary.monthlyIncome * 0.2)}</span>
                </p>
                <p className="text-[11px] text-[var(--color-text-muted)] mt-0.5">
                  Cần giảm {formatVND(whatIf.paymentReductionNeeded)}/tháng -{' '}
                  <Link to="/debts/repayment" className="text-blue-400 hover:text-blue-300 font-bold transition-colors">
                    xem kế hoạch trả nợ →
                  </Link>
                </p>
              </div>
