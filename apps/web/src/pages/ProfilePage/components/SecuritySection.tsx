...
import { Shield, ShieldCheck, ShieldAlert, QrCode, Copy, Check, AlertTriangle, Key, Download } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border-2 transition-all ${user?.isTwoFactorEnabled ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' : 'bg-slate-500/10 border-slate-500/30 text-slate-400'}`}>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">
              {user?.isTwoFactorEnabled ? 'Tài khoản đang được bảo mật tối đa.' : 'Nâng cao bảo mật cho tài khoản của bạn.'}
            </p>
          </div>
        </div>
        ...
        {(isSettingUp || isDisabling) && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => { if (!loading) { setIsSettingUp(false); setIsDisabling(false); } }}
              ...
              {isDisabling ? (
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-slate-400 mt-2">Nhập mã OTP từ ứng dụng của bạn để xác nhận yêu cầu này.</p>
                  </div>
                  ...
                  {step === 'qr' && setupData && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-400 mt-2">Sử dụng Google Authenticator hoặc Authy để quét mã này.</p>
                      </div>
                      ...
                      <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Hoặc nhập mã thủ công</p>
                        <div className="flex items-center justify-between">
                          <code className="text-indigo-400 font-mono font-bold tracking-wider">{setupData.secret}</code>
                          <button onClick={() => copyToClipboard(setupData.secret)} className="text-slate-500 hover:text-white transition-colors">
                            <Copy size={16} />
                          </button>
                        </div>
                      </div>
                  ...
                  {step === 'verify' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-400 mt-2">Nhập mã 6 chữ số vừa xuất hiện trong ứng dụng của bạn.</p>
                      </div>
                  ...
                  {step === 'backup' && backupCodes && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-400 mt-2">Hãy lưu các mã này ở nơi an toàn. Mỗi mã chỉ sử dụng được 1 lần.</p>
                      </div>
                      ...
                      <div className="flex gap-4">
                        ...
                        <button 
                          onClick={() => { setIsSettingUp(false); setStep('info'); }}
