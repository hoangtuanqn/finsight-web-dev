...
import { ChevronRight, Lock, ShieldCheck, Sparkles, Zap } from 'lucide-react';
import { Link } from 'react-router-dom';
import heroMockup from '../../../assets/hero-mockup.png';
import { useAuth } from '../../../context/AuthContext';
import { GradientText, Spotlight, useIsMobile } from './Shared';
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl font-black text-slate-900 dark:text-white leading-[1.1] mb-6 tracking-tighter">
            Làm chủ{' '}
            <GradientText from="from-blue-600" to="to-cyan-500">
              tài chính
            </GradientText>
            ,<br />
            xóa tan nỗi lo{' '}
          ...
          <p className="text-xl text-slate-600 dark:text-slate-400 mb-10 max-w-lg font-medium leading-relaxed">
            FinSight bóc tách mọi loại phí ẩn, tự động hóa kế hoạch trả nợ và phân bổ tài sản thông minh bằng trí tuệ
            nhân tạo thế hệ mới.
          </p>
          ...
          <div className="flex flex-wrap gap-4 mb-12">
            {isLoggedIn ? (
              <Link
                to="/home"
                className="group relative flex items-center gap-2 px-8 py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-[0_0_30px_rgba(37,99,235,0.3)] hover:shadow-[0_0_50px_rgba(37,99,235,0.5)] transition-all"
              >
              ...
              <>
                ...
                <Link
                  to="/login"
                  className="flex items-center gap-2 px-8 py-4 bg-white/50 dark:bg-white/5 text-slate-900 dark:text-white font-bold rounded-2xl border border-slate-200 dark:border-white/10 backdrop-blur-md hover:bg-white/80 dark:hover:bg-white/10 transition-all"
                >
                  Xem Demo
                </Link>
              </>
          ...
          <div className="flex flex-wrap gap-8">
            {[
              ...
              <div
                key={i}
                className="flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
              >
