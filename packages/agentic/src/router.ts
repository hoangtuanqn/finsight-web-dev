...
const KEYWORD_ROUTES = [
  ...
  {
    keywords: ['nợ bao nhiêu', 'tháng này trả', 'tình trạng nợ', 'khoản nợ của tôi', 'dti của tôi'],
...
export const routeIntent = async (query: string): Promise<string> => {
  ...
  try {
    ...
    const response = await Promise.race([
      model.invoke([new SystemMessage(promptText), new HumanMessage('Phân tích intent của câu trên.')]),
      new Promise<any>((_, reject) => setTimeout(() => reject(new Error('ROUTER_TIMEOUT')), ROUTER_TIMEOUT_MS)),
    ]);
