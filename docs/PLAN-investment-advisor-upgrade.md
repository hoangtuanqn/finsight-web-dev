...
**Audit dynamic source cho biểu đồ cột 2026-04-29**

Phạm vi audit này chỉ tính các dữ liệu thực sự được dùng để vẽ chart trong `SmartAssetGuide` qua `GET /api/investment/asset-history`. Các mảng metadata/allowlist như `STOCK_UNIVERSE`, tên card, badge, note, hoặc fallback snapshot hiện tại không được tính là dữ liệu chart. Tiết kiệm chưa có nguồn lịch sử kiểm chứng nên hiện không bật chart, tránh vẽ dữ liệu giả.

| Nhóm/card có chart | Endpoint request | Nguồn runtime | Range user chọn | Dynamic đến tháng/ngày hiện tại? | Ghi chú |
|---|---|---|---|---|---|
| Chứng khoán/ETF VN | `asset=stocks&ticker=<ALLOWLIST>&months=6/12/18` | Yahoo Finance chart API qua `fetchAssetHistory(ticker)` với `interval=1mo&range=5y` | 6/12/18 tháng | Có | Backend lấy 5 năm gần nhất rồi `slice(-months)`; ticker chỉ allowlist để không biến API thành proxy tự do. |
| Vàng thế giới `GC=F` | `asset=gold&source=world&months=6/12/18` | Yahoo Finance chart API `GC=F` qua `fetchAssetHistory()` | 6/12/18 tháng | Có | Dữ liệu monthly close động; cache Redis 24h nếu Redis khả dụng. |
| Vàng miếng SJC | `asset=gold&source=sjc&days=7/14/30` | `https://www.vang.today/api/prices?type=VNGSJC&days=<range>` | 7/14/30 ngày | Có | Nguồn public hiện chỉ dùng được history ngắn hạn; không giả lập 6/12/18 tháng. |
| Nhẫn tròn trơn | `asset=gold&source=ring&days=7/14/30` | `https://www.vang.today/api/prices?type=BT9999NTT&days=<range>` | 7/14/30 ngày | Có | Parser lấy giá bán cuối ngày, quy đổi về VND/chỉ. |
| Vàng thế giới quy đổi | `asset=gold&source=world&sourceType=proxy&months=6/12/18` | Proxy Yahoo `GC=F` | 6/12/18 tháng | Có, nhưng là proxy | UI phải hiển thị nhãn `Tham chiếu`, không coi là lịch sử trực tiếp của sản phẩm Việt Nam. |
| Tích lũy vàng DCA | `asset=gold&source=sjc&sourceType=proxy&days=7/14/30` | Proxy `vang.today` SJC | 7/14/30 ngày | Có, nhưng là proxy | Đây là chiến lược, không phải sản phẩm có giá riêng; UI đã ghi proxy. |
| TPCP Việt Nam 5Y/10Y/15Y | `asset=bonds&source=vn_gov_5y|10y|15y&months=6/12/18` | `vietnamBondHistory.service` tự sinh ngày đấu thầu từ `new Date()`, thử URL VBMA `en/vi`, parse yield 5Y/10Y/15Y | 6/12/18 tháng | Có | Không hardcode từng tháng; cache memory 12h. Nếu VBMA lỗi/không đủ điểm thì endpoint trả lỗi/no-data thay vì bịa chart. |
| Quỹ trái phiếu dùng benchmark | `asset=bonds&source=vn_gov_10y&sourceType=proxy&months=6/12/18` | Proxy TPCP Việt Nam 10Y từ VBMA | 6/12/18 tháng | Có, nhưng là proxy | Chưa có NAV history trực tiếp cho từng quỹ; UI phải hiển thị `Tham chiếu/Benchmark`. |
| Tiết kiệm | Không có `historySource` | Chưa có collector/backfill lịch sử | Không bật chart | Không áp dụng | `SAVINGS_DATA` là snapshot hiện tại, không được dùng để vẽ chart lịch sử. Cần collector/backfill trước khi bật chart. |
