...
import { Mail, ShieldCheck, Globe, Info, HelpCircle, ChevronRight } from 'lucide-react';
          ...
          <div className="lg:col-span-2 flex flex-col gap-8">
            <div className="flex flex-col gap-6">
              ...
              <p className="text-lg text-[var(--color-text-secondary)] leading-relaxed font-medium max-w-sm">
                Nâng tầm quản trị tài chính cá nhân với trí tuệ nhân tạo. FinSight giúp bạn kiến tạo sự tự do và bứt phá mọi giới hạn.
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="w-10 h-10 rounded-full border-2 border-[var(--color-bg-primary)] bg-[var(--color-bg-secondary)] flex items-center justify-center text-[10px] font-black text-blue-400 shadow-xl">
                    U{i}
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col gap-6 items-start text-left">
            ...
            <nav className="flex flex-col gap-4 items-start w-full">
              {['Dashboard', 'Thị trường', 'Phân tích AI', 'Giao dịch'].map(item => (
                <span key={item} className="text-[14px] font-semibold text-[var(--color-text-muted)] hover:text-blue-400 transition-all cursor-pointer flex items-center gap-0 group">
        ...
        <div className="mt-24 pt-12 border-t border-[var(--color-border)] flex flex-col md:flex-row justify-between items-end gap-12">
          <div className="flex flex-col gap-5 max-w-md w-full">
            ...
            <p className="text-[14px] text-[var(--color-text-muted)] font-medium leading-relaxed">
              © {new Date().getFullYear()} FinSight Ecosystem. Tất cả quyền được bảo lưu. Dữ liệu được bảo mật bởi hạ tầng đám mây phân tán của Golden Hands.
            </p>
          </div>

          <div className="flex flex-col items-center md:items-end gap-5 w-full md:w-auto">
            <div className="text-center md:text-right">
              <p className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-[0.5em] mb-3 opacity-60">Architected & Engineered by</p>
              <h2 className="text-5xl font-black text-gradient tracking-tighter uppercase mb-1 leading-none">Golden Hands</h2>
              <p className="text-[12px] font-black text-blue-500/50 tracking-[0.3em] uppercase">Premium Software Craftsmanship</p>
            </div>
