  ...
  return (
    <div className="relative bg-slate-900/60 backdrop-blur-xl border border-white/5 p-6 rounded-3xl shadow-[0_8px_32px_-8px_rgba(0,0,0,0.5)] overflow-hidden group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        {items.map((item, i) => (
          <div key={i} className="relative flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-colors">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                {item.extra && (
                  <span className="text-[10px] font-medium text-slate-500">{item.extra}</span>
                )}
              </div>
              {item.change !== undefined && (
                <div className={`flex items-center gap-1 mt-0.5 text-xs font-semibold ${item.change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
