...
import { RefreshCw, Smartphone, CheckCircle, AlertCircle, Loader2, ShieldCheck } from 'lucide-react';
...
export default function QRCodeLogin({ onLoginSuccess }) {
  ...
  const { data: qrData, refetch: refetchQR, isLoading: isGenerating, isError: isGenerateError } = useQuery({
          ...
          <motion.div key="expired" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-4 py-6">
            <div className="relative w-48 h-48 rounded-3xl bg-slate-100 dark:bg-white/5 flex items-center justify-center border-2 border-dashed border-slate-300 dark:border-white/10">
               <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-3xl z-10 p-6 text-center">
                  ...
                  <button onClick={handleRefresh} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all cursor-pointer">
          ...
          <motion.div key="qr" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center w-full">
              ...
              <div className="relative p-5 bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-xl overflow-hidden">
                {currentStatus === 'scanned' ? (
                  <div className="w-48 h-48 flex flex-col items-center justify-center text-center gap-3">
                    ...
                    <p className="text-[11px] font-bold text-slate-800 dark:text-white uppercase tracking-tight">Đã kết nối thiết bị</p>
            ...
            <div className="flex flex-col items-center gap-3 w-full">
               ...
               <p className="text-[11px] text-slate-500 dark:text-slate-400 text-center max-w-[240px] leading-relaxed">
                 Dùng <span className="font-black text-blue-500">Camera</span> hoặc <span className="font-black text-blue-500">Zalo</span> quét mã để đăng nhập an toàn.
               </p>
            </div>
          </motion.div>
        )}
