...
function calcFutureValue(capital, monthlyAdd, rate, years) {
...
function getProfileRates(profile = {}) {
  return {
...
export function getAllocationFromSource(source = {}) {
  return Object.fromEntries(
    ASSET_KEYS.map(key => [key, toNumber(source?.[key])])
  );
}

export function buildPortfolioBreakdown(allocation, profile = {}, providedBreakdown = null) {
...
export function buildPieData(allocation) {
  return ASSET_KEYS
...
function buildLegacyProjection(allocation, profile = {}) {
  const capital = toNumber(profile.capital);
  ...
  const inflationRate = profile.inflationRate !== undefined
    ...
    : 0.035;
  const rates = getProfileRates(profile);
...
function chooseProjection(projection, allocation, profile) {
...
export function buildProjectionData(projection, profile = {}) {
  const capital = toNumber(profile.capital);
  const monthlyAdd = toNumber(profile.monthlyAdd);
  const inflationRate = profile.inflationRate !== undefined
    ? toNumber(profile.inflationRate) / 100
...
export function buildMonteCarloData(projection = null, profile = {}) {
...
export function normalizeAllocationAnalysis(apiData = {}, profile = {}) {
...
export function normalizeStrategy(strategy = {}, profile = {}) {
